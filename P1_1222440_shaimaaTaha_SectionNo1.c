...
void addNewWord(words k,char m[size]) ;
    ...
    // Load the input file which contains the initial text.
     ...
     case 5:
        if (loaded){
                ...
                addNewWord(newText,temp->newWordes);
     ...
     case 6:
         if (loaded){
                ...
                if (temp2->operation==0){// if the operation insert in the undo then  add from the text
                        addNewWord(newText,temp2->newWordes); // add to the text

                push(temp2->newWordes,1,newText->count,undoStack); // push to the undo stack
                     //  sortingWord(newText);

                }else{
                      deleteWord(newText,temp2->newWordes); // delete
                    push(temp2->newWordes,0,newText->count,undoStack); // push to undo stack
...
while (temp!=NULL){
        ...
        stack curentElement=createStack();
        curentElement=top(s); // store the top to push into another stack
        push(curentElement->newWordes,curentElement->operation,curentElement->index,curentElement); // push
        pop(s);
...
if (b){
 ...
 addNewWord(newText,forPush); // to the original text
...
sortingWord(newText);
    ...
    void addNewWord(words L,char m[size]) {
    words temp = createList();
    strcpy(temp->word, m);  //  copy the name
  temp->count=counter;  // assign count
  if (isEmpty2(L)) {
        L->next = temp;
        counter++;
    } else {
